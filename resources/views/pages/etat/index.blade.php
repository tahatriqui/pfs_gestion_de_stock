            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                        <h6 class="text-white text-capitalize ps-3">Les etats</h6>
                        ...
                        <table class="table align-items-center mb-0">
                            ...
                            <tbody>
                                ...
                                @forelse($etats as $key => $etat)
                                    ...
                                    <td>
                                        <a href="{{ route('etat.modifier', $etat->id) }}" type="button"
                                            class="btn btn-success">modifier</a>
                                        <form action="{{ route('etat.delete', $etat->id) }}" class="d-inline" method="POST"
                                            id="delete-form-{{ $etat->id }}">
                                            @csrf
                                            @method('DELETE')
                                            <button onclick="confirmDelete('delete-form-{{ $etat->id }}')"
                                                type="button" class="btn btn-danger">supprime</button>
                                        </form>
                                        <a href="{{ route('etat.filtre', ['id' => $etat->id, 'filtre' => $etat->etats]) }}"
                                            class="btn btn-dark">filtrer par etat</a>
                                    </td>
                                    </tr>
                                @empty
                                    <tr>
